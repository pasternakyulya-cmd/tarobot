...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "🪄 «Помощь Оракула» — твой личный советчик.\n"
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("oracle_state") == "after_free":
        ...
        payment_msg = (
            ...
            "Если готов(а) — нажми кнопку ниже. Как только пройдёт оплата, я сразу приступлю к твоему запросу! 💫"
        )
        ...
        keyboard = [[InlineKeyboardButton("🔮 Оплатить 49₽", url=payment_49.confirmation.confirmation_url)]]
    ...
    if context.user_data.get("oracle_state") == "waiting_question":
        ...
        if str(user_id) not in free_used:
            ...
            # Одно сообщение: и про бесплатку, и про дальнейшие вопросы
            await update.message.reply_text(
                ...
                "Если потом захочешь уточнить что-то ещё — просто напиши новый вопрос.\n"
            )
            ...
            # помним, что бесплатка уже использована, и следующая реплика будет платной
        ...
        keyboard = [
            [
                InlineKeyboardButton("🔮 Оплатить 49₽", url=payment_49.confirmation.confirmation_url),
            ]
            # [
    ...
    if t == BTN_ORACLE.lower() or ("помощь" in t and "оракула" in t):
        oracle_message = (
            "Привет! Я здесь, чтобы помочь разобраться в твоей ситуации. 🪄\n\n"
            "Расскажи, что тебя волнует — и я сразу приступлю к анализу.\n\n"
            "Можешь спросить о чём угодно: о таро-раскладе, отношениях, работе или любом важном выборе. "
            "Чем подробнее опишешь — тем глубже я смогу заглянуть в суть вопроса.\n\n"
            "Жду твоего сообщения! ✨"
        )

        # путь к фото
        photo_path = "media/oracle_start.jpg"
